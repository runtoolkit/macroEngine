...
public final class ScheduleService {
	public record Entry(long nextTick, int interval, boolean repeat, Consumer<MinecraftServer> action) {}

	private final Map<String, Entry> entries = new ConcurrentHashMap<>();
	private long tick;

	public void schedule(String key, int delayTicks, Consumer<MinecraftServer> action) {
		entries.put(key, new Entry(tick + Math.max(0, delayTicks), 0, false, action));
	}
	...
	public void scheduleRepeating(String key, int intervalTicks, Consumer<MinecraftServer> action) {
		...
		entries.put(key, new Entry(tick + iv, iv, true, action));
	}
	...
	public void tick(MinecraftServer server) {
		...
		while (it.hasNext()) {
			...
			if (entry.repeat()) {
				e.setValue(new Entry(tick + entry.interval(), entry.interval(), true, entry.action()));
			} else {
				it.remove();
			}
		}
